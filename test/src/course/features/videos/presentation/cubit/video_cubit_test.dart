...
import 'package:education_app/src/course/features/videos/presentation/cubit/video_state.dart';
...
void main() {
  ...
  setUp(() {
    ...
    videoCubit = VideoCubit(addVideo: addVideo, getVideos: getVideos);
  ...
  test('initial state should be [VideoInitial]', () {
    expect(videoCubit.state, const VideoInitial());
  });

  group('addVideo', () {
    blocTest<VideoCubit, VideoState>(
      'emit [AddingVideo, VideoAdded] when addVideo is called',
      build: () {
    ...
    blocTest<VideoCubit, VideoState>(
      'emit [AddingVideo, VideoError] when addVideo is called and fails',
      build: () {
        when(() => addVideo(any())).thenAnswer(
          (_) async => Left(
      ...
      expect: () => [
        const AddingVideo(),
        const VideoError('500 Error: Server Failure'),
      ],
      verify: (_) {
        verify(() => addVideo(tVideo)).called(1);
        verifyNoMoreInteractions(addVideo);
  ...
  group('getVideos', () {
    blocTest<VideoCubit, VideoState>(
      'emit [LoadingVideo, LoadedVideos] when getVideos is called',
      build: () {
      ...
      verify: (_) {
        verify(() => getVideos(tVideo.courseId)).called(1);
        verifyNoMoreInteractions(getVideos);
      },
    );

    blocTest<VideoCubit, VideoState>(
      'emit [LoadingVideo, VideoError] when getVideos is called and fails',
      build: () {
