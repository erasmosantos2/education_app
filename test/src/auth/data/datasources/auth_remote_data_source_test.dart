...
void main() {
  ...
  group('SignIn', () {
    ...
    test(
      'Should throw [ServerException] when user is null '
      'after signing in ',
      () async {
        final emptyCredential = MockUserCredential();
        when(
          () => authClient.signInWithEmailAndPassword(
            email: any(named: 'email'),
            password: any(named: 'password'),
          ),
        ).thenAnswer((_) async => emptyCredential);
  ...
  group('updateUser', () {
    ...
    test(
      'Should update user displayName successfully when no [Serverexception] '
      ' is thrown ',
      () async {
        when(
        ...
        expect(userData['fullName'], tFullName);
      },
    );
